...
- After every agent run, in addition to summarizing findings, include a short "Meta-eval on this run" section covering:
  ...
  - Recommendations — concrete tactics to improve the agent prompt / workflow
- When delegating to the agent, pass the self-eval directive in the prompt: "Include a self-evaluation section at the end (strengths / weaknesses / recommendations)" — so the agent can score itself, not just me scoring it externally.
