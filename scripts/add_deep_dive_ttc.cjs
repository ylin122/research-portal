  ...
  sections: [
    ...
    },
    {
      ...
      key_numbers: [
        ...
        { label: "AIME vs TTC", value: "Log-linear" }
      ]
    },
    {
...
(async () => {
  console.log("Inserting deep dive: " + dive.title);
  const { data, error } = await supabase.from("deep_dives").upsert(dive).select();
  if (error) {
    console.error("ERROR:", error);
    process.exit(1);
  }
  console.log("OK — inserted/updated row:");
