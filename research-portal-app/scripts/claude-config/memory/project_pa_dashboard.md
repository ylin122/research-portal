...
- Forward P/E = `forwardPE ?? priceEpsCurrentYear` (NTM/forward earnings is canonical — matches `api/quote.mjs:32-34`). `priceEpsCurrentYear` is current FY (partly historical), used only as fallback when `forwardPE` is missing.
